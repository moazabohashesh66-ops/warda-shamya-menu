...
export default function CategoryGrid({ menuData }: { menuData: any[] }) {
  const router = useRouter();

  // التأكد إن menuData Array قبل استخدام map
  if (!menuData || !Array.isArray(menuData) || menuData.length === 0) {
    return (
      <section className="py-20 px-4 bg-[#1a0e0a]">
        <div className="text-center text-white/50">
          <p className="text-xl">⚠️ لا توجد بيانات حالياً</p>
          <p className="text-sm mt-2 text-white/30">تأكد من اتصال قاعدة البيانات</p>
        </div>
      </section>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <button
            ...
            className="text-white/20 hover:text-white/60 transition-colors duration-300 text-sm font-light tracking-wider"
          >
            ⚙️ إدارة المطعم
          </button>
        </motion.div>
